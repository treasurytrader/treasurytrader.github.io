...
IV. History:
2017.09.21  1.7      Add Tick Sender & Fix Ansi PostMessageA -> Unicode
PostMessageW 2014.03.10  1.6      Modified to support build 600 and later
...
extension. added safe checking for PeriodMultiplier. 2005.12.04  1.3      Fixed
missing data when there is large amount of data loaded in several blocks, and
support auto updating when new history is loaded. 2005.11.29  1.2 Additional fix
for missing data and server changing. 2005.11.29  1.1      Fixed missing partial
data after restart. Reinitialize after changing server or data corrupted.
2005.11.28  1.0      Initial release
*/

extern double Version = 1.7; // code version
extern string BuildInfo = "2017.09.21 by c1";
extern int PeriodMultiplier = 1; // new period multiplier factor
extern int OutputCSVFile = 0;    // also output CSV file?
extern int UpdateInterval =
    0; // update interval in milliseconds, zero means update real-time.
...
int init() {
  ...
  string suffix = "";
  if (ShiftTiming != 0)
    suffix = StringConcatenate(suffix, "s", ShiftTiming);
  if (GMTShift != 0)
    suffix = StringConcatenate(suffix, "g", GMTShift);
  if (OmitDigit != 0)
    suffix = StringConcatenate(suffix, "o" /*,OmitDigit*/);
  // MySymbol =Symbol() + "_" + suffix;
  MySymbol = Symbol() + suffix;
  if (StringLen(MySymbol) > 11)
    MySymbol = StringConcatenate(
        StringSubstr(MySymbol, 0, 11 - StringLen(suffix)), suffix);
...
int OpenHistoryFile() {
  ...
  if (OutputCSVFile != OUTPUT_CSV_ONLY) {
    FileHandle =
        FileOpenHistory(name + ".hst", FILE_BIN | FILE_WRITE | FILE_SHARE_READ);
    if (FileHandle < 0)
      return (-1);
  }
  if (OutputCSVFile != OUTPUT_HST_ONLY) {
    CSVHandle =
        FileOpen(name + ".csv",
                 FILE_CSV | FILE_WRITE | FILE_SHARE_READ | FILE_ANSI, ',');
    if (CSVHandle < 0)
      return (-1);
  }
  return (0);
...
void WriteHistoryData() {
  ...
  if (CSVHandle >= 0) {
    ...
    FileWrite(CSVHandle, TimeToStr(i_time, TIME_DATE),
              TimeToStr(i_time, TIME_MINUTES), DoubleToStr(d_open, i_digits),
              DoubleToStr(d_high, i_digits), DoubleToStr(d_low, i_digits),
              DoubleToStr(d_close, i_digits), d_volume);
  }
}
...
int UpdateHistoryFile(int start_pos, bool init = false) {
  ...
  if (init) {
    ...
    if (CSVHandle >= 0)
      csv_fpos = FileTell(CSVHandle);
  } else {
    i = start_pos;
    if (FileHandle >= 0)
      FileSeek(FileHandle, last_fpos, SEEK_SET);
    if (CSVHandle >= 0)
      FileSeek(CSVHandle, csv_fpos, SEEK_SET);
  }
  if (i < 0)
    return (-1);
...
int UpdateChartWindow() {
  ...
  if (hwnd != 0) {
    ...
    if (PostMessageW(hwnd, WM_COMMAND, CHART_CMD_UPDATE_DATA, 0) == 0) {
...
int CheckNewData() {
  ...
  // server changed? check this and reinit to prevent wrong data while changing
  // server.
  if (LastServer != serv) {
